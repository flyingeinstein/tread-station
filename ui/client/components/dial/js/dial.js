...
import {ButtonIndicator,ButtonGroupIndicator} from "./button-indicator.js";
import {GraduatedIndicator} from "./graduated-indicator.js";
...
export function Dial(_container, options) {
	...
	this.scales.speed = scaleLinear()
		.domain([1, 9])
		.range([dial_begin, dial_end]
		);
	this.scales.progress = scaleLinear()
		.domain([0, 1])
...
Dial.prototype.zoom = function(value, transitionTimeMs)
{
	...
	if(transitionTimeMs) {
		...
		var interpolator = interpolate(this.options.zoom, value);
